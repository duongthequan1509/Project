...
<html lang="en">
  <head>
    <title>Sự Kiện-Thể Thao Điện Tử</title>
      ...
      <div class="col-md-8">
        	...
        	<ul class="nav nav-tabs tintuc" role="tablist"  style="height:34px;">
                <li role="presentation" class="active" style="height:34px;">
                    <a href="#home_news_tab_1" role="tab" data-toggle="tab" style="height:34px; font-size:16px;font-family:Arial; line-height:0.5">SỰ KIỆN ĐANG DIỄN RA</a>
                </li>
            </ul>
        </div>
        	  <?php
			            foreach ($sukien as $value) {
							echo '<div class="container-fluid news2" style="padding-bottom:25px;">';
							echo ' <table>';
            			    echo '<tr >';
                			echo '<td ><a href="http://thethaodt.vn/tin-tuc/'. $value['id'] . '/'. $value['url'] . '"><img " src="../../public/upload/user/'. $value['img'] . '"></a></td>';
                			echo '<td><p><b><a href="http://thethaodt.vn/tin-tuc/'. $value['id'] . '/'. $value['url'] . '">['. $value['type_dr'] . ']'. $value['title'] . '</a></b><br /><br /><i>'. $value['sumay'] . '</i></p></td>';
                			echo '<tr >';
							echo '  </table>';
							echo '</div>';
